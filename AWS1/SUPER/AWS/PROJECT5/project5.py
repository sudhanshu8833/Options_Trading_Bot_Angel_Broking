...
def main():
    ...
    if signal=='buy':
        market_order()
        logging.info(f"New long position initiated at {price1}")
        bot.sendMessage(1039725953,f"New long position initiated at {price1}")

    if signal=='sell':
        market_order1()
        logging.info(f"New short position initiated at {price2}")
        bot.sendMessage(1039725953,f"New short position initiated at {price2}")
    
    if signal=="squareoffsell":
        market_order1()
        logging.info(f"long position squared of at {price2}")
        position=""
        bot.sendMessage(1039725953,f"long position squared of at {price2}")
        bot.sendMessage(1039725953,f" profit of {((price2-price1)/price1)*100*20}")
