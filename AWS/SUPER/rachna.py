...
# %%
time.sleep(1860-(time.time()-times))
...
for ticker in tickers:
    ohlc_dict=candle(ticker)
    high[ticker]=ohlc_dict['High'].iloc[-3]
    low[ticker]=ohlc_dict['Low'].iloc[-3]
...
bot.sendDocument(1039725953, document=open('rachna.csv', 'rb'))