...
bot.sendMessage(504938312,"selected stocks for today")
bot.sendMessage(504938312,stocks)
...
def market_order(instrument,investment):
    ...
    bot.sendMessage(504938312,f"long position generated for {instrument} at {ltp}")
    logging.info(f"long position generated for {instrument} at {ltp}")
    # try:
...
def main():
    ...
    for ticker in stocks:
        ...
        logging.info(f"analyzzing for {ticker}")
        logging.info(ltp)
...
# %%
time.sleep(900-(time.time()-times1))
start_time=time.time()

timeout=start_time+60*60*6+15*60
times=time.time()
...
while time.time()<=timeout:
    ...
    try:
        ...
        logging.info("passthrough at ",time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())))
